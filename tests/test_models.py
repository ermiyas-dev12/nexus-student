...
class TestModels:
    def test_user_creation(self, app):
        with app.app_context():
            user = User(username="alice", email="alice@example.com", password=generate_password_hash("secret"))
            db.session.add(user)
            db.session.commit()
    ...
    def test_chat_history_creation(self, app):
        with app.app_context():
            chat = ChatHistory(session_id="sess-abc", message="Hello", response="Hi there!")
            db.session.add(chat)
            db.session.commit()
